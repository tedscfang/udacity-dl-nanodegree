...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        #hidden_error_term = hidden_error * hidden_outputs * (1-hiddee_outputs)
        hidden_error_term = hidden_error * hidden_outputs * (1 - hidden_outputs)
        # Weight step (input to hidden)
        delta_weights_i_h += X[None,:].T.dot(hidden_error_term[None,:])
        #delta_weights_i_h += hidden_error_term * X[:,None]
        # Weight step (hidden to output)
        delta_weights_h_o += hidden_outputs[None,:].T.dot(output_error_term[None,:])
        #output_error_term.dot(hidden_outputs)
